            ...
            <section class="card riwayat-petugas-section fade-in">
                ...
                <div class="riwayat-list">
                    ...
                    <?php
                        ...
                        if ($izin['status'] === 'Diizinkan') { $class = 'status-success'; }
                        elseif ($izin['status'] === 'Bolos') { $class = 'status-danger'; }
                        elseif ($izin['status'] === 'Telah Kembali Ke Sekolah') { $class = 'status-success-light'; }
                        elseif ($izin['status'] === 'Menunggu') { $class = 'status-pending'; $status_text = 'MENUNGGU'; }

                        // Highlight untuk yang Bolos
                        $highlight_class = isset($izin['is_bolos']) ? 'item-highlight' : '';
                    ?>
                    <div class="izin-petugas-item <?= $highlight_class ?>">
                        <h4><?= $izin['nama'] ?> <span class="status <?= $class ?>"><?= strtoupper($status_text) ?></span></h4>
